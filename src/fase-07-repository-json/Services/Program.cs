...
using Fase07.RepositoryJson.Services;

Console.WriteLine("Fase 7 — Repository JSON (CurrencyRate domain)");

// arquivo JSON no diretório base do app
var path = Path.Combine(AppContext.BaseDirectory, "currency_rates.json");
Console.WriteLine($"Arquivo JSON: {path}");

IRepository<CurrencyRate, int> repo = new JsonCurrencyRateRepository(path);
