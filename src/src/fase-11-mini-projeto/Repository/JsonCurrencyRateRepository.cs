...
public sealed class JsonCurrencyRateRepository : IReadRepository<CurrencyRate,int>, IWriteRepository<CurrencyRate,int>
{
    ...
    public CurrencyRate? GetById(int id) => Load().FirstOrDefault(r => r.Id == id);

    public IReadOnlyList<CurrencyRate> ListAll() => Load();
